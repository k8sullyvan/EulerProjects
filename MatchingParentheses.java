...
public class MatchingParentheses {
	...
	private Boolean isMatch(String pattern) {
		//Convert the String into only a char array of the parentheses, removing the other characters
		...
		for (char c : digits) {
			...
			if (!parenthesesMatch(c)) parenthesesStack.push(c);
		}
		//All parentheses have been paired if only the white space character remains
	...
	private Boolean parenthesesMatch(char c) {
		...
		switch(c) {
			case ')': 	pairMatch = parenthesesStack.peek().equals('(');
						break;
			case ']': 	pairMatch = parenthesesStack.peek().equals('[');
						break;
			case '}': 	pairMatch = parenthesesStack.peek().equals('{');
		...
		if (pairMatch) parenthesesStack.pop();
		return pairMatch;
	}
}
